...
def Run_test(tests):
    ...
    #K
    for i in tests:
        j=i;l=i;m=i;n=i;o=i;p=i
        newDir = "k"+str(i)+", Ev"+str(j)+", Ft"+str(l)+", Cnd"+str(m)+", Res"+str(n)+", In"+str(o)+", Ex"+str(p)
        print(newDir)
        if not os.path.exists(os.path.join("Tests/Detailed/all",newDir)):
            os.mkdir(os.path.join("Tests/Detailed/all",newDir))
        csv_file_path = os.path.join("Tests/Detailed/all", newDir, "data.csv")
        dataToStore =["numberOfOptimalTraces","modelsExecutionTime","exploredNodes","totalTime"]
        with open(os.path.join("Tests/Detailed/all", "avgs.csv"), 'w', newline='') as avgs_file:
            with open(csv_file_path, 'w', newline='') as csv_file:
                ...
                with open(os.path.join("Tests/Detailed/all",newDir, "tests.txt"), 'w', newline='') as tests_file:
                    graphs = ""
                    while (validGraph<5):
